\startenvironment cv

\setuppapersize[letter]
\setuplayout[backspace=5.5cm, leftmargin=3cm, rightedge=0cm, width=13cm]
\setuppagenumbering[location=footer]
\setupinmargin[align=right]

\define[2]\DrawLine
{#2\inmargin{\fillinline[width=2.8cm, color={\getvariable{metadata}{maincolor}}, rulethickness=3.5pt]}}

\definehead[ResumeHead][subject]
\setuphead[ResumeHead][style=\bfa, color={\getvariable{metadata}{maincolor}}, command=\margintitle{\fillinline[width=2.8cm, color={\getvariable{metadata}{maincolor}}, rulethickness=3.5pt]}]

\startluacode
require("cv")
\stopluacode

\def\ResumeEntry[#1]{\ctxlua{
                userdata.ResumeEntry('#1')}
}
\def\ResumeShortEntry[#1]{\ctxlua{
                userdata.ResumeShortEntry('#1')}
}
\def\ResumeTitle[#1]{\ctxlua{
                userdata.ResumeTitle('#1')}
}

\stopenvironment